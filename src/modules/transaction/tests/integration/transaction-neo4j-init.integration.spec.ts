...
import { TransactionNeo4jInitializer } from '../../projections/transaction-neo4j.projection';

const describeIntegration =
  process.env.RUN_INTEGRATION_TESTS === 'true' ? describe : describe.skip;

/**
 * Transaction Neo4j Initialization Integration Tests
 *
 * Tests the Neo4j constraint and index creation for Transaction module.
 *
 * Prerequisites:
 * - Docker services running: `docker-compose -f docker-compose.test.yml up -d`
 * - Neo4j available at bolt://localhost:7687
 *
 * Run: `npm run test:integration -- --testPathPattern=transaction-neo4j-init.integration.spec`
 */
describeIntegration('TransactionNeo4jInitializer Integration', () => {
  ...
  beforeAll(async () => {
    try {
      module = await Test.createTestingModule({
        ...
        providers: [TransactionNeo4jInitializer],
      }).compile();

      await module.init();
      neo4jService = module.get<Neo4jService>(Neo4jService);
      initializer = module.get<TransactionNeo4jInitializer>(TransactionNeo4jInitializer);
    } catch (error) {
      console.warn(
  ...
  describe('constraint and index creation', () => {
    it('should create all expected constraints and indexes', async () => {
      ...
      expect(await constraintExists('transaction_id_unique')).toBe(true);
      expect(await indexExists('transaction_status_index')).toBe(true);
      expect(await indexExists('transaction_type_index')).toBe(true);
      expect(await indexExists('transaction_createdAt_index')).toBe(true);
    });
  });
  ...
  describe('connection failure handling', () => {
    it('should throw error when Neo4j is unavailable', async () => {
      ...
      const badInitializer =
        badModule.get<TransactionNeo4jInitializer>(TransactionNeo4jInitializer);
