...
import { OrganizationNeo4jInitializer } from '../../projections/organization-neo4j.projection';

const describeIntegration =
  process.env.RUN_INTEGRATION_TESTS === 'true' ? describe : describe.skip;

/**
 * Organization Neo4j Initialization Integration Tests
 *
 * Tests the Neo4j constraint and index creation for Organization module.
 *
 * Prerequisites:
 * - Docker services running: `docker-compose -f docker-compose.test.yml up -d`
 * - Neo4j available at bolt://localhost:7687
 *
 * Run: `npm run test:integration -- --testPathPattern=organization-neo4j-init.integration.spec`
 */
describeIntegration('OrganizationNeo4jInitializer Integration', () => {
  ...
  beforeAll(async () => {
    try {
      module = await Test.createTestingModule({
        ...
        providers: [OrganizationNeo4jInitializer],
      }).compile();

      await module.init();
      neo4jService = module.get<Neo4jService>(Neo4jService);
      initializer = module.get<OrganizationNeo4jInitializer>(OrganizationNeo4jInitializer);
    } catch (error) {
      console.warn(
  ...
  describe('constraint and index creation', () => {
    it('should create all expected constraints and indexes', async () => {
      ...
      expect(await constraintExists('organization_id_unique')).toBe(true);
      expect(await indexExists('organization_type_index')).toBe(true);
      expect(await indexExists('organization_status_index')).toBe(true);
      expect(await indexExists('organization_createdAt_index')).toBe(true);
    });
  });
  ...
  describe('connection failure handling', () => {
    it('should throw error when Neo4j is unavailable', async () => {
      ...
      const badInitializer =
        badModule.get<OrganizationNeo4jInitializer>(OrganizationNeo4jInitializer);
