...
/**
 * Actor Neo4j Initialization Integration Tests
 *
 * Tests the Neo4j constraint and index creation for Actor module.
 *
 * Prerequisites:
 * - Docker services running: `docker-compose -f docker-compose.test.yml up -d`
 * - Neo4j available at bolt://localhost:7687
 *
 * Run: `npm run test:integration -- --testPathPattern=actor-neo4j-init.integration.spec`
 */
describeIntegration('ActorNeo4jInitializer Integration', () => {
  ...
  beforeAll(async () => {
    try {
      module = await Test.createTestingModule({
        ...
        providers: [ActorNeo4jInitializer],
      }).compile();
  ...
  describe('constraint and index creation', () => {
    it('should create all expected constraints and indexes', async () => {
      ...
      expect(await constraintExists('actor_id_unique')).toBe(true);
      expect(await indexExists('actor_type_index')).toBe(true);
      expect(await indexExists('actor_workspaceId_index')).toBe(true);
    });
  });
  ...
  describe('connection failure handling', () => {
    it('should throw error when Neo4j is unavailable', async () => {
      ...
      const badInitializer = badModule.get<ActorNeo4jInitializer>(ActorNeo4jInitializer);
