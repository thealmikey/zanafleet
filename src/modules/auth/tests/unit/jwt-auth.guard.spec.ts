...
describe('JwtAuthGuard', () => {
  ...
  describe('handleRequest', () => {
    ...
    it('should throw UnauthorizedException when user is null (missing token)', () => {
      ...
      expect(() => guard.handleRequest(null, null, undefined, mockContext)).toThrow('Unauthorized');
    });
    ...
    it('should throw UnauthorizedException with "Token has expired" for expired tokens', () => {
      ...
      expect(() => guard.handleRequest(null, null, expiredError, mockContext)).toThrow(
        UnauthorizedException
      );
      expect(() => guard.handleRequest(null, null, expiredError, mockContext)).toThrow(
        'Token has expired'
    ...
    it('should throw UnauthorizedException with "Invalid token" for malformed tokens', () => {
      ...
      expect(() => guard.handleRequest(null, null, invalidError, mockContext)).toThrow(
        UnauthorizedException
      );
      expect(() => guard.handleRequest(null, null, invalidError, mockContext)).toThrow(
        'Invalid token'
    ...
    it('should throw UnauthorizedException with "Token not yet valid" for NotBeforeError', () => {
      ...
      expect(() => guard.handleRequest(null, null, notBeforeError, mockContext)).toThrow(
        UnauthorizedException
      );
      expect(() => guard.handleRequest(null, null, notBeforeError, mockContext)).toThrow(
        'Token not yet valid'
    ...
    it('should throw UnauthorizedException with info message when provided', () => {
      ...
      expect(() => guard.handleRequest(null, null, customError, mockContext)).toThrow(
        UnauthorizedException
      );
      expect(() => guard.handleRequest(null, null, customError, mockContext)).toThrow(
        'Custom auth error'
    ...
    it('should throw UnauthorizedException when error is provided even with valid user', () => {
      ...
      expect(() => guard.handleRequest(error, { id: 'test' }, undefined, mockContext)).toThrow(
        UnauthorizedException
      );
    });
