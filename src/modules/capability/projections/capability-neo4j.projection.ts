...
@Injectable()
export class CapabilityNeo4jProjection
  implements IEventHandler<CapabilityCreatedEventV1>
{
  ...
  async handle(event: CapabilityCreatedEventV1): Promise<void> {
    this.logger.log(
      `Handling CapabilityCreatedEvent-V1 for capability: ${event.capabilityId}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `Capability node projected to Neo4j: ${event.capabilityId}`,
      );
    } catch (error: unknown) {
      const err = error instanceof Error ? error : new Error(String(error));
      this.logger.error(
        `Failed to project capability to Neo4j: ${err.message}`,
        err.stack,
      );
      throw err;
    } finally {
...
export class CapabilityNeo4jInitializer {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const err = error instanceof Error ? error : new Error(String(error));
      this.logger.error(
        `Failed to initialize Capability Neo4j schema: ${err.message}`,
        err.stack,
      );
      throw err;
    } finally {
