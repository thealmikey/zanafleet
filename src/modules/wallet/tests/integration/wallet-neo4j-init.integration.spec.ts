...
import { WalletNeo4jInitializer } from '../../projections/wallet-neo4j.projection';

const describeIntegration = process.env.RUN_INTEGRATION_TESTS === 'true' ? describe : describe.skip;

/**
 * Wallet Neo4j Initialization Integration Tests
 *
 * Tests the Neo4j constraint and index creation for Wallet module.
 *
 * Prerequisites:
 * - Docker services running: `docker-compose -f docker-compose.test.yml up -d`
 * - Neo4j available at bolt://localhost:7687
 *
 * Run: `npm run test:integration -- --testPathPattern=wallet-neo4j-init.integration.spec`
 */
describeIntegration('WalletNeo4jInitializer Integration', () => {
  ...
  beforeAll(async () => {
    try {
      module = await Test.createTestingModule({
        ...
        providers: [WalletNeo4jInitializer],
      }).compile();

      await module.init();
      neo4jService = module.get<Neo4jService>(Neo4jService);
      initializer = module.get<WalletNeo4jInitializer>(WalletNeo4jInitializer);
    } catch (error) {
      console.warn(
  ...
  describe('constraint and index creation', () => {
    it('should create all expected constraints and indexes', async () => {
      ...
      expect(await constraintExists('wallet_id_unique')).toBe(true);
      expect(await indexExists('wallet_ownerId_index')).toBe(true);
      expect(await indexExists('wallet_type_index')).toBe(true);
      expect(await indexExists('wallet_ownerType_index')).toBe(true);
    });
  });
  ...
  describe('connection failure handling', () => {
    it('should throw error when Neo4j is unavailable', async () => {
      ...
      const badInitializer = badModule.get<WalletNeo4jInitializer>(WalletNeo4jInitializer);
