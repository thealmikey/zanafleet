...
describe('3rd Party Integration - Asset Platform API (e2e)', () => {
  ...
  describe('Asset Management', () => {
    ...
    it('06: Should return 404 for non-existent asset', () => {
      return request(app.getHttpServer()).get('/assets/non-existent-id').expect(404);
    });
  ...
  describe('Trip Management', () => {
    ...
    it('17: Should retrieve trip by ID', () => {
      return request(app.getHttpServer()).get(`/assets/trips/${createdTripId}`).expect(200);
    });
  ...
  describe('Error Handling & Validation', () => {
    ...
    it('27: Should return 404 for non-existent bundle', () => {
      return request(app.getHttpServer()).get('/bundles/non-existent-bundle-id').expect(404);
    });

    it('28: Should return 404 for non-existent trip', () => {
      return request(app.getHttpServer()).get('/assets/trips/non-existent-trip-id').expect(404);
    });
  ...
  describe('Batch Operations', () => {
    it('31: Should create multiple assets in sequence', async () => {
      ...
      for (const asset of assets) {
        await request(app.getHttpServer()).post('/assets').send(asset).expect(201);
      }
    });
  ...
  describe('Edge Cases & Boundary Tests', () => {
    it('37: Should handle empty match query', () => {
      return request(app.getHttpServer()).post('/assets/match').send({ input: '' }).expect(200);
    });
