...
} from '@api/modules/organization/events/organization-created.event';
import { OrganizationNeo4jProjection } from '@api/modules/organization/projections/organization-neo4j.projection';
  ...
  async handleOrganizationEvent(
    ...
    try {
      if (data.eventType === 'OrganizationCreatedEvent-V1') {
        ...
        const event: OrganizationCreatedEventV1 = OrganizationCreatedEventV1.fromJSON(
          data as unknown as OrganizationCreatedEventV1JSON
        );
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        await this.projection.handle(event);
        this.eventLogger.logProcessed(event as unknown as BaseEvent, 'OrganizationNeo4jProjection');
      }
    } catch (error: unknown) {
