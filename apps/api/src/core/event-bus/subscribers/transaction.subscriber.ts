import { TransactionCreatedEventV1 } from '@api/modules/transaction/events/transaction-created.event';
import { TransactionNeo4jProjection } from '@api/modules/transaction/projections/transaction-neo4j.projection';
  ...
  async handleTransactionEvent(
    ...
    try {
      if (data.eventType === 'TransactionCreatedEvent-V1') {
        ...
        const event: TransactionCreatedEventV1 = TransactionCreatedEventV1.fromJSON(
          eventData as Parameters<typeof TransactionCreatedEventV1.fromJSON>[0]
        );
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        await this.projection.handle(event);
        this.eventLogger.logProcessed(event as unknown as BaseEvent, 'TransactionNeo4jProjection');
      }
    } catch (error) {
