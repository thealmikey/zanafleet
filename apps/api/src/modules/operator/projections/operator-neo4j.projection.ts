...
export class OperatorNeo4jProjection implements IEventHandler<OperatorOnboardedEventV1> {
  ...
  async handle(event: OperatorOnboardedEventV1): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(
        `Operator node and relationship to Actor created/updated in Neo4j: ${event.operatorId}`
      );
    } catch (error) {
...
export class OperatorNeo4jInitializer {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      await session.run(
        'CREATE CONSTRAINT operator_id_unique IF NOT EXISTS FOR (o:Operator) REQUIRE o.id IS UNIQUE'
      );
      await session.run(
        'CREATE INDEX operator_actor_id_index IF NOT EXISTS FOR (o:Operator) ON (o.actorId)'
      );
      this.logger.log('Neo4j constraints/indexes for Operator initialized');
    } catch (error) {
      this.logger.error(
        `Failed to initialize Neo4j for Operator: ${
