...
describe('ConfidenceThresholdService', () => {
  ...
  describe('getThreshold', () => {
    ...
    it('should return 0.90 for Order capabilities', () => {
      expect(service.getThreshold('CreateOrder')).toBe(0.90);
      expect(service.getThreshold('UpdateOrder')).toBe(0.90);
      expect(service.getThreshold('CancelOrder')).toBe(0.90);
    });
    ...
    it('should return 0.70 for unknown capabilities', () => {
      expect(service.getThreshold('UnknownCapability')).toBe(0.70);
      expect(service.getThreshold('DoSomething')).toBe(0.70);
    });

    it('should return 0.60 for Search capabilities', () => {
      expect(service.getThreshold('Search')).toBe(0.60);
    });

    it('should return 0.50 for View and List capabilities', () => {
      expect(service.getThreshold('View')).toBe(0.50);
      expect(service.getThreshold('List')).toBe(0.50);
    });
  ...
  describe('shouldPropose', () => {
    it('should return true when confidence meets threshold', () => {
      ...
      expect(service.shouldPropose(0.90, 'CreateOrder')).toBe(true);
    ...
    it('should use default threshold for unknown capabilities', () => {
      expect(service.shouldPropose(0.70, 'Unknown')).toBe(true);
  ...
  describe('shouldClarify', () => {
    it('should return true when confidence is below threshold but above minimum bar', () => {
      ...
      expect(service.shouldClarify(0.80, 'Payment')).toBe(true);
      
      // Order: threshold 0.90, minimum bar ~0.63
      expect(service.shouldClarify(0.70, 'CreateOrder')).toBe(true);
    });
    ...
    it('should return false when confidence is too low (below minimum bar)', () => {
      ...
      expect(service.shouldClarify(0.60, 'Payment')).toBe(false);
    });
  });
  ...
  describe('shouldReject', () => {
    it('should return true when confidence is below minimum bar', () => {
      // Payment: threshold 0.95, minimum bar ~0.665
      expect(service.shouldReject(0.60, 'Payment')).toBe(true);
      expect(service.shouldReject(0.50, 'Payment')).toBe(true);
    });

    it('should return false when confidence meets or exceeds minimum bar', () => {
      // Payment: threshold 0.95, minimum bar ~0.665
      expect(service.shouldReject(0.70, 'Payment')).toBe(false);
  ...
  describe('getRecommendedAction', () => {
    it('should return "propose" when confidence meets threshold', () => {
      ...
      expect(service.getRecommendedAction(0.90, 'CreateOrder')).toBe('propose');
    });

    it('should return "clarify" when confidence is between minimum bar and threshold', () => {
      expect(service.getRecommendedAction(0.80, 'Payment')).toBe('clarify');
      expect(service.getRecommendedAction(0.70, 'CreateOrder')).toBe('clarify');
    });

    it('should return "reject" when confidence is below minimum bar', () => {
      expect(service.getRecommendedAction(0.60, 'Payment')).toBe('reject');
      expect(service.getRecommendedAction(0.50, 'CreateOrder')).toBe('reject');
    });
  });

  describe('setThreshold', () => {
    it('should set a custom threshold', () => {
      service.setThreshold('CustomCapability', 0.80);
      expect(service.getThreshold('CustomCapability')).toBe(0.80);
    });
  ...
  describe('resetThreshold', () => {
    it('should reset threshold to default', () => {
      service.setThreshold('Payment', 0.50);
      service.resetThreshold('Payment');
      expect(service.getThreshold('Payment')).toBe(0.95);
    });

    it('should use default for unknown capabilities', () => {
      service.setThreshold('UnknownCap', 0.50);
      service.resetThreshold('UnknownCap');
      expect(service.getThreshold('UnknownCap')).toBe(0.70);
    });
  });
