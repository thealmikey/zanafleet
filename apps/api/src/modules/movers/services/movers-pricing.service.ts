...
import {
  GEO_PROVIDER,
  GeoPoint,
} from '../../location-intelligence/providers/geo-provider.interface';
import {
  MovingQuoteRequest,
  MovingQuote,
  AvailableSlot,
...
export class MoversPricingService {
  ...
  async calculateQuote(request: MovingQuoteRequest): Promise<Omit<MovingQuote, 'vehicles'>> {
    this.logger.log(
      `Calculating quote for move: ${request.movingFrom.formattedAddress} → ${request.movingTo.formattedAddress}`
    );
  ...
  private calculateBasePrice(
    ...
    const sizeMultiplier =
      Math.max(currentConfig.capacityCubicMeters, destinationConfig.capacityCubicMeters) / 10;
