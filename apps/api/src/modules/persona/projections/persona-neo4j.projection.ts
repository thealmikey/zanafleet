...
@Injectable()
export class PersonaNeo4jProjection implements IEventHandler<PersonaCreatedEventV1> {
  ...
  async handle(event: PersonaCreatedEventV1): Promise<void> {
    this.logger.log(`Handling PersonaCreatedEvent-V1 for persona: ${event.personaId}`);
    ...
    try {
      ...
      this.logger.debug(`Persona node created/updated in Neo4j: ${event.personaId}`);
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      this.logger.error(`Failed to project persona to Neo4j: ${err.message}`, err.stack);
      throw error;
    } finally {
      await session.close();
    }
