...
export class MatchingService {
    ...
    async matchAssets(input: string) {
        ...
        return {
            ...
            message: matchedAssets.length > 0
    ...
    private async estimateRequirements(input: string): Promise<MatchingRequirements> {
        ...
        // Skill detection
        if (lowerInput.includes('refrigerated') || lowerInput.includes('perishable') || lowerInput.includes('cold')) {
            skills.push('Cold Chain');
        }
        if (lowerInput.includes('lift') || lowerInput.includes('heavy') || lowerInput.includes('mover')) {
            skills.push('Heavy Lifting');
        }
        if (lowerInput.includes('cross-border') || lowerInput.includes('uganda') || lowerInput.includes('tanzania')) {
            skills.push('Cross-Border Transit');
        }

        // Bundling detection (multiple entities)
        if (lowerInput.includes('office') || lowerInput.includes('relocation') || lowerInput.includes('project')) {
            isBundle = true;
        }

        // Simple heuristic for demo purposes
        if (lowerInput.includes('house') || lowerInput.includes('moving') || lowerInput.includes('furniture')) {
            return {
        ...
        if (lowerInput.includes('package') || lowerInput.includes('food') || lowerInput.includes('small')) {
            return {
                suggestedType: AssetType.VEHICLE,
        ...
        if (lowerInput.includes('warehouse') || lowerInput.includes('store') || lowerInput.includes('storage')) {
            return {
   ...
   */
    private async findMatchingAssets(requirements: MatchingRequirements): Promise<AssetEntity[]> {
        try {
            ...
            this.logger.warn(`Search provider failed: ${error instanceof Error ? error.message : String(error)}. Falling back to DB query.`);
        }
