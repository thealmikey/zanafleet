...
describe('SearchProjectionService', () => {
    ...
    it('1. should index OrderCreatedEventV1 correctly (Mapping)', async () => {
        ...
        expect(mockSearchProvider.index).toHaveBeenCalledWith(expect.objectContaining({
            entityId: 'order-123',
            entityType: 'Order',
    ...
    it('5. should catch and log errors during indexing (Error Handling)', async () => {
        ...
        const event = new OrderCreatedEventV1({
            eventId: 'evt-5', orderId: 'o-1', businessId: 'b-1', deliveryId: null,
            itemSummary: 'test', status: OrderStatus.Pending, paymentStatus: PaymentStatus.Pending,
            scheduledTime: null, createdAt: new Date()
        });

        await service.handle(event);

        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to process event'), expect.any(String));
    });

    it('6. should preserve metadata fields correctly (Metadata Preservation)', async () => {
        const event = new OrderCreatedEventV1({
            eventId: 'evt-6', orderId: 'o-6', businessId: 'b-6', deliveryId: null,
            itemSummary: 'test', status: OrderStatus.Fulfilled, paymentStatus: PaymentStatus.Succeeded,
            totalAmount: 100, currency: 'KES', scheduledTime: null, createdAt: new Date()
        });
    ...
    it('7. should generate consistent titles for entities (Title Generation)', async () => {
        const event = new OrderCreatedEventV1({
            eventId: 'evt-7', orderId: 'abcdefgh-1234', businessId: 'b-7', deliveryId: null,
            itemSummary: 'test', status: OrderStatus.Pending, paymentStatus: PaymentStatus.Pending,
            scheduledTime: null, createdAt: new Date()
        });
    ...
    it('8. should handle null or empty summaries gracefully (Description Sanitization)', async () => {
        const event = new OrderCreatedEventV1({
            eventId: 'evt-8', orderId: 'o-8', businessId: 'b-8', deliveryId: null,
            itemSummary: null, status: OrderStatus.Pending, paymentStatus: PaymentStatus.Pending,
            scheduledTime: null, createdAt: new Date()
        });
    ...
    it('9. should handle rapid event bursts for the same entity (Concurrency)', async () => {
        const event1 = new DeliveryStateTransitionedEventV1({
            eventId: 'evt-9a', deliveryId: 'del-burst', previousState: DeliveryStatus.Requested, newState: DeliveryStatus.PickedUp, transitionedAt: new Date()
        });
        const event2 = new DeliveryStateTransitionedEventV1({
            eventId: 'evt-9b', deliveryId: 'del-burst', previousState: DeliveryStatus.PickedUp, newState: DeliveryStatus.Delivered, transitionedAt: new Date()
        });
