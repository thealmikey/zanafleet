...
export class ActivitySeederService implements OnModuleInit {
    ...
    async seedActivity(): Promise<void> {
        ...
        const businessExists = await this.businessRepository.findOne({ where: { id: this.TEST_BUSINESS_ID } });
        if (!businessExists) {
        ...
        const orderCount = await this.orderRepository.count({ where: { customerPhone: '+254700123456' } });
        if (orderCount > 0) {
        ...
        for (const item of statuses) {
            ...
            const order = OrderEntity.fromDomain({
                ...
                currency: 'KES',
                paymentStatus: item.order === OrderStatus.Fulfilled ? PaymentStatus.Succeeded : PaymentStatus.Pending,
                createdAt: date,
            });
