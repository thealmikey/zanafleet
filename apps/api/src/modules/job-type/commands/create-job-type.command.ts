/**
 * CreateJobTypeCommand
 *
 * Command object representing the intent to create a new job type
...
/**
 * Zod validation schema for CreateJobTypeCommand
 */
export const CreateJobTypeCommandSchema = z.object({
  workspaceId: z.string().uuid(),
  name: z.string().trim().min(1).max(255),
  description: z.string().trim().max(1000).optional().or(z.null()),
  vertical: z.nativeEnum(Vertical),
  mode: z.nativeEnum(JobTypeMode).default(JobTypeMode.INTERNAL),
  status: z.nativeEnum(JobTypeStatus).default(JobTypeStatus.ACTIVE),
  workflowDefinitionId: z.string().uuid().optional().or(z.null()),
  assignmentStrategy: z
    .object({
      type: z.enum(['manual', 'auto', 'bidding', 'hybrid']),
      autoAssignmentRules: z
        .object({
  ...
  slaRules: z
    ...
    .default({}),
  supportsMultipleWorkers: z.boolean().default(false),
  supportsMultipleDestinations: z.boolean().default(false),
  verticalSpecificSettings: z.record(z.unknown()).optional(),
  workerConfigs: z.array(WorkerConfigInputSchema).default([]),
  metadataFields: z.array(MetadataFieldInputSchema).default([]),
});

export type CreateJobTypeCommandInput = z.infer<typeof CreateJobTypeCommandSchema>;

/**
 * CreateJobTypeCommand
 */
export class CreateJobTypeCommand {
  ...
  readonly workerConfigs: CreateJobTypeCommandInput['workerConfigs'];
  readonly metadataFields: CreateJobTypeCommandInput['metadataFields'];

  constructor(input: CreateJobTypeCommandInput) {
    ...
    this.workflowDefinitionId = input.workflowDefinitionId ?? null;
    ...
    this.verticalSpecificSettings = input.verticalSpecificSettings ?? null;
   ...
   */
  static validate(input: unknown): CreateJobTypeCommandInput {
    return CreateJobTypeCommandSchema.parse(input);
  }

  /**
   * Safe validation - returns result object instead of throwing
   */
  static safeValidate(input: unknown): z.SafeParseReturnType<unknown, CreateJobTypeCommandInput> {
    return CreateJobTypeCommandSchema.safeParse(input);
  }
}
