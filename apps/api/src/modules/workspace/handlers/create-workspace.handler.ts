...
export class CreateWorkspaceCommandHandler implements ICommandHandler<CreateWorkspaceCommand> {
  ...
  async execute(command: CreateWorkspaceCommand): Promise<string> {
    ...
    try {
      ...
      if (!organization) {
        this.logger.warn(`Organization not found: ${command.orgId}`);
        throw new NotFoundException(`Organization with ID '${command.orgId}' does not exist`);
      }

      this.logger.debug(`Organization validated: ${command.orgId}`);
      ...
      this.logger.debug(`Workspace persisted to PostgreSQL: ${workspaceId}`);
      ...
      this.logger.log(`WorkspaceCreatedEvent-V1 emitted to event bus: ${eventId}`);
      ...
      this.logger.error(`Failed to create workspace: ${err.message}`, err.stack);
      throw error;
    }
  }
}
