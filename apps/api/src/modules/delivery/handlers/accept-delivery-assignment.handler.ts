  ...
  implements ICommandHandler<AcceptDeliveryAssignmentCommand>
{
  ...
  async execute(command: AcceptDeliveryAssignmentCommand): Promise<string> {
    ...
    if (
      existing.assignedRiderId === riderId &&
      existing.assignmentNotifiedAt != null
    ) {
    ...
    if (existing.status !== DeliveryStatus.Assigned && existing.status !== DeliveryStatus.PickedUp && existing.status !== DeliveryStatus.InTransit) {
