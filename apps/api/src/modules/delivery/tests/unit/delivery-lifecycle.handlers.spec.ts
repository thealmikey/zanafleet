...
describe('Delivery Lifecycle Command Handlers', () => {
  ...
  it('assigns rider and publishes DeliveryAssignedEventV1', async () => {
    repo.findOneByOrFail
      ...
      .mockResolvedValueOnce(baseEntity({ assignedRiderId: 'r-1', assignedAt: new Date('2025-01-01T00:10:00.000Z'), status: DeliveryStatus.Assigned })) // after
  ...
  it('accepts delivery assignment and publishes DeliveryAssignedEventV1 with accepted=true', async () => {
    repo.findOneByOrFail
      .mockResolvedValueOnce(baseEntity({ assignedRiderId: 'r-1', assignedAt: new Date('2025-01-01T00:05:00.000Z') })) // before
      .mockResolvedValueOnce(baseEntity({ assignedRiderId: 'r-1', assignedAt: new Date('2025-01-01T00:05:00.000Z'), assignmentNotifiedAt: new Date('2025-01-01T00:06:00.000Z'), status: DeliveryStatus.Assigned })) // after
  ...
  it('marks delivery picked up and publishes DeliveryPickedUpEventV1', async () => {
    repo.findOneByOrFail
      ...
      .mockResolvedValueOnce(baseEntity({ status: DeliveryStatus.PickedUp, pickedUpAt: new Date('2025-01-01T00:20:00.000Z') })) // after
  ...
  it('marks delivery delivered and publishes DeliveryDeliveredEventV1', async () => {
    repo.findOneByOrFail
      ...
      .mockResolvedValueOnce(baseEntity({ status: DeliveryStatus.Delivered, deliveredAt: new Date('2025-01-01T00:45:00.000Z') })) // after
  ...
  it('cancels delivery and publishes DeliveryCancelledEventV1', async () => {
    repo.findOneByOrFail
      ...
      .mockResolvedValueOnce(baseEntity({ status: DeliveryStatus.Cancelled, cancelledAt: new Date('2025-01-01T00:50:00.000Z') })) // after
  ...
  it('records failed attempt and publishes DeliveryFailedEventV1', async () => {
    repo.findOneByOrFail
      .mockResolvedValueOnce(baseEntity()) // before
      .mockResolvedValueOnce(baseEntity({ attemptCount: 1, lastAttemptAt: new Date('2025-01-01T01:00:00.000Z') })) // after
